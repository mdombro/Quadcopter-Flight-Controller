...
void writePWM4(int32_t spd) {
	int32_t val = Map_f_in_int_out(spd, 0.0, 100.0, PWM_MIN_TICKS, PWM_MAX_TICKS);
	PWMPulseWidthSet(PWM1_BASE, PWM_OUT_3, val);
}
...
void ConfigurePWM() {
	...
	PWMPulseWidthSet(PWM1_BASE, PWM_OUT_3, 5000);
	//
	...
	// Enable the outputs.
	//
	PWMOutputState(PWM1_BASE, PWM_OUT_2_BIT | PWM_OUT_5_BIT | PWM_OUT_6_BIT | PWM_OUT_3_BIT, true);   //  | PWM_OUT_1_BIT
}
